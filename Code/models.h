...
class farfield_circle : public mr2d::model
{
	...
	public:
	void
	discretise (int n, double points[][3])
	{ for(int i = 0; i < n; ++i) function(points[i][0] = (double)i/n, &points[i][1]); }
	/* === Model uniform discretisation === */
};
...
class model_cst_naca : public mr2d::model
{
	...
	{
		int i;
		for(i = 0; i < (n+1)/2; ++i) function(points[i][0] = (cos((double)i/n * 2*M_PI) - 1) / -4.0, &points[i][1]);
		for(     ; i < n;       ++i) function(points[i][0] = (cos((double)i/n * 2*M_PI) + 3) /  4.0, &points[i][1]);
	}
	/* === Model cosine discretisation === */
	...
	/* === Euclidian distance === */
	private:
	double _euclidian(double p1[2], double p2[2])
