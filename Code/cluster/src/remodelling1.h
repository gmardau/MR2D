...
_assess_vertex (_Vertex v1, _Vertex v2, unsigned int i = 0)
{
	...
	for( ; ; ) {
		...
		/* Get next triangle */
		_aux_vertices[0].push_back(v1); _aux_vertices[0].push_back(v3_t.first);
...
{
	/* If orientation is no longer positive considering that neighbour vertices cannot be adjusted - not adjust */
	if((v2->_type == 1 || v3->_type == 1) && geometry::orientation(v1_x, v2->_x, v3->_x) <= 0) return false;
